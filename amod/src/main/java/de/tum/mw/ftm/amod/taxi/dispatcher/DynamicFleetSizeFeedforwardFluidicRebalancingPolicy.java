...
public class DynamicFleetSizeFeedforwardFluidicRebalancingPolicy extends DynamicFleetSizeDispatcher {
    ...
    public DynamicFleetSizeFeedforwardFluidicRebalancingPolicy(Config config, //
    ...
    public void redispatch(double now) {
        ...
        if (round_now % rebalancingPeriod == 0 && travelData.coversTime(round_now)) {
            Tensor rebalancingRate = travelData.getAlphaRateAtTime((int) round_now);
            ...
            Map<VirtualNode<Link>, List<RoboTaxi>> availableVehicles = this.getVirtualNodeDivertableNotRebalancingRoboTaxis();
        ...
        if (round_now % dispatchPeriod == 0) {
            ...
            Tensor printVals = bipartiteMatcher.executePickup(this, getDivertableUnassignedRoboTaxis(), getUnassignedPassengerRequests(), distanceFunction, network);
        }
    }
    ...
    public static class Factory implements AVDispatcherFactory {
        ...
        public AmodeusDispatcher createDispatcher(ModalProviders.InstanceGetter inject) {
            ...
            return new DynamicFleetSizeFeedforwardFluidicRebalancingPolicy(config, operatorConfig, travelTime, router, eventsManager, network, virtualNetwork, //
                    abstractVirtualNodeDest, abstractVehicleDestMatcher, travelData, db, dynamicFleetSize);
        }
    }
}
