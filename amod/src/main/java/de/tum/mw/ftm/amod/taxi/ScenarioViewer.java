...
import de.tum.mw.ftm.amod.viewer.PredictionGridLayer;
...
/* package */ enum ScenarioViewer {
    ...
    public static void run(File workingDirectory) throws FileNotFoundException, IOException {
        ...
        /** load options */
        Config config = ConfigUtils.loadConfig(scenarioOptions.getSimulationConfigName(), new FTMConfigGroup());
        ...
        SimulationModule simulationModule = new SimulationModule(network);
        PredictionGridLayer predictionGridLayer= new PredictionGridLayer(amodeusComponent, simulationModule.provideUniformGridWithBorderPredictions(config));
        amodeusComponent.addLayer(predictionGridLayer);
        /** starting the viewer */
