...
/* package */ enum VehicleContainerCompiler {
    ...
    public static VehicleContainer compile(RoboTaxi roboTaxi, List<LinkStatusPair> linkStatusPairs) {
        ...
        if (linkStatusPairs.get(linkStatusPairs.size() - 1).roboTaxiStatus == roboTaxi.getStatus()) {
            ...
            List<Integer> linkTrace = linkStatusPairs.stream().map(p -> p.link).map(Link::getId).mapToInt(Id::index).boxed().collect(Collectors.toList());
            linkTrace.add(linkTrace.get(linkTrace.size() - 1));
            vc.linkTrace = ArrayUtils.toPrimitive(linkTrace.toArray(Integer[]::new));
        }
