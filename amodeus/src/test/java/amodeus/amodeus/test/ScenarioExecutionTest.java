...
public class ScenarioExecutionTest {
    ...
    public void testC_Analysis() {
        ...
        scalarAssert.add((Scalar) Quantity.of(2838.50307, "km").map(Round._5), (Scalar) ate.getDistancElement().totalDistance.map(Round._5));
        ...
        scalarAssert.add((Scalar) Quantity.of(1522.18704, "km").map(Round._5), (Scalar) ate.getDistancElement().totalDistanceRebal.map(Round._5));
        scalarAssert.add((Scalar) RealScalar.of(0.36759).map(Round._5), (Scalar) ate.getDistancElement().totalDistanceRatio.map(Round._5));
    ...
    public void testD_Viewer() throws IOException {
        if (!UserName.is("travis") && !GraphicsEnvironment.isHeadless()) {
            // run scenario viewer
