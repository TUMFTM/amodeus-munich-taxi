...
public class NominatimQueryTester extends TestCase {
    ...
    public void testSimple() throws Exception {
        ...
        if (latLong != null) {
            double latitude = latLong.Get(0).number().doubleValue();
            double longitude = latLong.Get(1).number().doubleValue();
            assertEquals(7.4448531, latitude, 0.001);
            assertEquals(46.9472594, longitude, 0.001);
        } else {
            fail("Cannot retrieve Coordinates from Query!");
        }
    }
}
